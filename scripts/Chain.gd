...
var SPEED = 11	# The speed with which the chain moves
var GRAVITY = .04
...
func _ready():
	...
	ui_node.connect("hook_gravity_change", self, "handle_hook_gravity_change")
	ui_node.connect("hook_speed_change", self, "handle_hook_speed_change")
...
func _process(_delta: float) -> void:
	...
	$Tip.rotation = direction.angle() + deg2rad(90) #self.position.angle_to_point(tip_loc) - deg2rad(90)
	print($Tip.rotation)
...
func _physics_process(_delta: float) -> void:
	...
	if flying:
		...
		direction.y += GRAVITY # gravity
		if $Tip.move_and_collide(direction * SPEED):
			hooked = true	# Got something!
			flying = false	# Not flying anymore
	tip = $Tip.global_position	# set `tip` as starting position for next frame

func handle_hook_gravity_change(value):
	GRAVITY = value
	
	
func handle_hook_speed_change(value):
	SPEED = value
